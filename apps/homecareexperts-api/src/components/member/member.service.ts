...
export class MemberService {
	...
	// public async resetNotificationCount(userId: ObjectId): Promise<void> {
	// 	await this.memberModel.findByIdAndUpdate(userId, { notificationCount: 0 });
	// }
	...
	public async likeTargetMember(memberId: ObjectId, likeRefId: ObjectId): Promise<Member> {
		...
		const authorDetails = await this.getMember(memberId, memberId);
		...
		const notificationInput: NotificationInput = {
			...
			notificationDesc: `${authorName} has ${modifier > 0 ? 'liked' : 'unliked'} your profile.`,
			authorId: memberId,
		...
		// const getNotifications = await this.notificationService.getNotifications(target._id);

		// console.log('getNotifications', getNotifications);

		// const markNotificationsAsRead = await this.notificationService.markNotificationsAsRead(target._id);

		// console.log('markNotificationsAsRead', markNotificationsAsRead);

		// const getUnreadNotificationCount = await this.notificationService.getUnreadNotificationCount(target._id);
